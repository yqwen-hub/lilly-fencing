...
all_time = find(r"(\d+\s+victories,\s+\d+\s+losses,\s+\d+\.?\d*%\s+win ratio)", history_text)

season = find(r"(2025.?2026.*?\d+\s+victories,\s+\d+\s+losses,\s+\d+\.?\d*%\s+win ratio)", history_text)

de_strength = find(r"DE strength estimate\s*(\d+)", strength_text)
pool_strength = find(r"Pool strength estimate\s*(\d+)", strength_text)
